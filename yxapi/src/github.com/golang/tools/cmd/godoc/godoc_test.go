...
tryagain:
	for _, test := range []struct{ url, pattern string }{
		...
		{"/src/yxapi/main.go", "L2.*package .*Package docs for app"},
		{"/src/yxapi/main.go", "L3.*import .*Package docs for lib.*lib"},
		{"/src/yxapi/main.go", "L4.*func main.*package lib.*lib.*var lib.V lib.T.*V"},
	} {
