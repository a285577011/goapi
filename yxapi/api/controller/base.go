...
func (this *BaseController) Init() bool {
	token := this.GetRequest().GetQuery("token")
	if token != "" {
		Redis := redis.GetRedisByDb("1")
		UsetTokenKey := "token_user:" + token
		RedisToken := Redis.Get(UsetTokenKey)
		if RedisToken != nil {
			Redis.ComDo("expire", UsetTokenKey, 604800)
			UserNo = RedisToken.(string)
		} else {
			UserNo = ""
		}
	} else {
