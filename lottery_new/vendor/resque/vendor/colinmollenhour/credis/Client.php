...
class Credis_Client {
     ...
     */
    public function __construct($host , $port, $timeout, $persistent , $db = 0, $password = '')
    {
        
//         $this->host = '127.0.0.1';
        if(YII_ENV_DEV){
            $this->host = '211.149.205.201';
            $this->port = 63790;
            $this->authPassword = 'goodluck';
        }else{
            $this->host = '27.155.105.165';
            $this->port = 63790;
            $this->authPassword = 'gula_lottery_redis';
        }
        $this->selectedDb = 2;
    ...
    protected function convertHost()
    {
        if (preg_match('#^(tcp|unix)://(.*)$#', $this->host, $matches)) {
            if($matches[1] == 'tcp') {
                ...
                $this->host = $matches[1];
                $this->port = (int) (isset($matches[3]) ? $matches[3] : 63790);
