...
class Controller extends Component implements ViewContextInterface
{
    ...
    public function beforeAction($action)
    {
        ...
        $this->storeCode = \Yii::$storeCode;
        $this->storeOperatorId = \Yii::$storeOperatorId;
     ...
     * @return 
     */
    public  function jsonResult($code,$msg,$data,$isCrpty = false,$third = false)
    {
        $respone = array(
            "code" => $code,
            'msg' => $msg,
        ...
        if($third) {
            $jsonRet = json_encode($respone,JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE |JSON_FORCE_OBJECT);
        }  else {
            $jsonRet = json_encode($respone,JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        }
        if($isCrpty){
            $des = new Des('gl_lottery','20180117');
            $jsonRet = $des->encrypt($jsonRet);
        }
        echo $jsonRet;
        \Yii::$app->end();
    }
