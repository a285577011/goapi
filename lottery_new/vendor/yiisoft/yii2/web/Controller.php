...
//             $d_api = $this->id;           //当前controller名
//             $d_action = $action->id;      //当前action名字
//             if($d_api == 'default'){
                
//             }else{
//                 $time = date("Y-m-d H:i:s");
//                 if (false == is_string($token) || $token == '0') {
//                     if (('login' == $d_action && 'user' == $d_api)
//                         || ('third_login' == $d_action && 'user' == $d_api)
//                         || ('start' == $d_action && 'user' == $d_api)
//                         || ('upload-picture' == $d_action && 'user' == $d_api)
//                         || ('register' == $d_action && 'user' == $d_api)
//                         || ('send_sms' == $d_action && 'user' == $d_api)
//                         || ('update-password' == $d_action && 'user' == $d_api)
//                         || ('pro-period' == $d_action && 'interface' == $d_api)
//                         ) {
//                             //这些接口不用验证token
//                         } else {
// //                             return $this->jsonResult(402, "user_ticket not found", '');
//                         }
//                 }
//             }
//             //验证token的合法性:从数据库或者redis中验证token返回user_id
//             if (is_string($token) && $token != '0') {
//                 $redisToken = \Yii::redisGet('token_user:'.$token);
//                 if(!empty($redisToken)){
//                     $this->custNo = $redisToken;
//                 }else{
//                     $connection = \Yii::$app->db;
//                     $nowTime = date('Y--m-d H:i:s');
//                     $user = UserToken::find()->select(['cust_no'])->where(['token'=>$token])->andWhere("'{$nowTime}' < expire_time")->asArray()->one();
//                     if(empty($user)){
//                         return $this->jsonResult(400, "该帐号登录失效，请重新登录", '');
//                     }             
//                     $this->custNo = $token['cust_no'];
//                 }
//             }
//             return true;
//         }
