...
class Request extends \yii\base\Request
{
     ...
     * 说明: 获取不能为空的参数
     * @author chenqiwei
     * @date 2018/2/11 上午10:26
     * @param
     * @return
     */
    public function post_nn($name = null, $defaultValue = null)
    {
        if ($name === null) {
            $ret = $this->getBodyParams();
        } else {
            $ret = $this->getBodyParam($name, $defaultValue);
        }
        if(empty($ret)){
            return \Yii::jsonError(109,'参数错误 '.$name.' 不能为空');
        }
        return $ret;
    }
