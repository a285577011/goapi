...
class SocketSync
{
     ...
     */
    public function __construct($host, $port, $recvTimeoutSec = 3, $recvTimeoutUsec = 750000, $sendTimeoutSec = 3, $sendTimeoutUsec = 100000)
    {
        $this->host = $host;
        $this->port = $port;
    ...
    public function read($len, $verifyExactLength = false)
    {
        ...
        if ($readable > 0) {
            ...
            while ($remainingBytes > 0) {
                ...
                if (strlen($chunk) === 0) {
                    ...
                    if (feof($this->stream)) {
                        $this->close();
                        throw new \Kafka\Exception('Unexpected EOF while reading '.$len.' bytes from stream (no data)',110);
                    }
                    // Otherwise wait for bytes
                    $readable = @stream_select($read, $null, $null, $this->recvTimeoutSec, $this->recvTimeoutUsec);
                    if ($readable !== 1) {
                        throw new \Kafka\Exception('Timed out reading socket while reading ' . $len . ' bytes with ' . $remainingBytes . ' bytes to go',600);
                    }
        ...
        $this->close();
        throw new \Kafka\Exception('Could not read '.$len.' bytes from stream (not readable);timeout'.$this->recvTimeoutSec);
    }
