...
use app\modules\sports\helpers\JXConstants;
use app\modules\sports\helpers\JXCalculation;

class JiangxiService {
    ...
    public function playOrder($custNo, $userId, $storeId, $storeCode, $source = 1, $sourceId = "", $boolStr = false, $outType) {
        ...
        $jxCode = $orderData['lottery_code']; //彩种code
...
//        }
        ...
        $betNums = '';
        $playName = [];
        $playParam = Constants::JX11X5_PLAYNAME;
        $price = Constants::PRICE;
        $lotArr = Constants::LOTTERY;
        $abbArr = Constants::LOTTERY_ABBREVI;
        $renP = JXConstants::REN_PUTONG;
        $renD = JXConstants::REN_DANTUO;
        $qianO = JXConstants::QIAN_ONE_ZHI;
        $qianTZ = JXConstants::QIAN_TWO_ZHI;
        $qianSZ = JXConstants::QIAN_THREE_ZHI;
        $qianZP = JXConstants::QIAN_ZU_PUTONG;
        $qianZD = JXConstants::QIAN_ZU_DANTUO;
        $ballNums = JXConstants::JX11X5_NUMS;
        $lotName = $lotArr[$jxCode];
        $abbName = $abbArr[$jxCode];
        $format = 'Y-m-d H:i:s';
        $createTime = date($format);
         foreach ($betCont as $key => $val) {
            if(in_array($val['play'], $renP)){
                $noteNums = JXCalculation::jx115Fun_RenPu($val['nums'], $ballNums[$val['play']]);
            }elseif (in_array($val['play'], $renD)) {
                $noteNums = JXCalculation::jx115Fun_RenDan($val['nums'], $ballNums[$val['play']]);
            }  elseif (in_array($val['play'], $qianO)) {
                $noteNums = JXCalculation::jx115Fun_QianOneZhi($val['nums']);
            }  elseif (in_array($val['play'], $qianTZ)) {
                $noteNums = JXCalculation::jx115Fun_QianTwoZhi($val['nums']);
            }  elseif (in_array($val['play'], $qianSZ)) {
                $noteNums = JXCalculation::jx115Fun_QianThreeZhi($val['nums']);
            }elseif (in_array($val['play'], $qianZP)) {
                $noteNums = JXCalculation::jx115Fun_QianZuPu($val['nums'], $ballNums[$val['play']]);
            }elseif (in_array($val['play'], $qianZD)) {
                $noteNums = JXCalculation::jx115Fun_QianZuDan($val['nums'], $ballNums[$val['play']]);
            } else {
        ...
        if ($total != $cost) {
         ...
         $insert = ['lottery_type' => $abbName, 'lottery_name' => $lotName, 'lottery_id' => $jxCode, 'play_code' => implode(',', $playCode), 'play_name' => implode(',', $playName), 'periods' => $periods,
...
//        print_r($insert);die;
    ...
    public function productSuborder($model) {
        ...
        foreach ($noteNums as $key => $nums) {
            $fun = 'jx115Note_' . $playCodes[$key];
            $ret = JXCalculation::$fun($nums);
            foreach ($ret as $k => $v) {
                $order[$n]['bet_val'] = $v;
