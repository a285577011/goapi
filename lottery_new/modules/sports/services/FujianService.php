...
use app\modules\sports\helpers\FJConstants;
use app\modules\sports\helpers\FJCalculation;
...
class FujianService {
    ...
    public function playOrder($custNo, $userId, $storeId, $storeCode, $source = 1, $sourceId = "", $boolStr = false, $outType) {
        ...
        $fjCode = $orderData['lottery_code']; //彩种code
...
//        }
        ...
        $betNums = '';
        $playName = [];
        $playParam = FJConstants::FJ11X5_PLAYNAME;
        $price = Constants::PRICE;
        $lotArr = Constants::LOTTERY;
        $abbArr = Constants::LOTTERY_ABBREVI;
        $renP = FJConstants::REN_PUTONG;
        $renD = FJConstants::REN_DANTUO;
        $qianO = FJConstants::QIAN_ONE_ZHI;
        $qianTZ = FJConstants::QIAN_TWO_ZHI;
        $qianSZ = FJConstants::QIAN_THREE_ZHI;
        $qianZP = FJConstants::QIAN_ZU_PUTONG;
        $qianZD = FJConstants::QIAN_ZU_DANTUO;
        $leS = FJConstants::LE_SAN;
        $leP = FJConstants::LE_PUTONG;
        $leD = FJConstants::LE_DANTUO;
        $ballNums = FJConstants::FJ11X5_NUMS;
        $lotName = $lotArr[$fjCode];
        $abbName = $abbArr[$fjCode];
        $format = 'Y-m-d H:i:s';
        $createTime = date($format);
        $singleCost = 0;
        foreach ($betCont as $val) {
            if(in_array($val['play'], $renP)){
                $noteNums = FJCalculation::fj115Fun_RenPu($val['nums'], $ballNums[$val['play']]);
                $singleCost += $noteNums * $price * $multiple;
            }elseif (in_array($val['play'], $renD)) {
                $noteNums = FJCalculation::fj115Fun_RenDan($val['nums'], $ballNums[$val['play']]);
                $singleCost += $noteNums * $price * $multiple;
            }  elseif (in_array($val['play'], $qianO)) {
                $noteNums = FJCalculation::fj115Fun_QianOneZhi($val['nums']);
                $singleCost += $noteNums * $price * $multiple;
            }  elseif (in_array($val['play'], $qianTZ)) {
                $noteNums = FJCalculation::fj115Fun_QianTwoZhi($val['nums']);
                $singleCost += $noteNums * $price * $multiple;
            }  elseif (in_array($val['play'], $qianSZ)) {
                $noteNums = FJCalculation::fj115Fun_QianThreeZhi($val['nums']);
                $singleCost += $noteNums * $price * $multiple;
            }elseif (in_array($val['play'], $qianZP)) {
                $noteNums = FJCalculation::fj115Fun_QianZuPu($val['nums'], $ballNums[$val['play']]);
                $singleCost += $noteNums * $price * $multiple;
            }elseif (in_array($val['play'], $qianZD)) {
                $noteNums = FJCalculation::fj115Fun_QianZuDan($val['nums'], $ballNums[$val['play']]);
                $singleCost += $noteNums * $price * $multiple;
            }elseif (in_array($val['play'], $leS)) {
                $noteNums = FJCalculation::fj115Fun_LeSan($val['nums']);
                $singleCost += $noteNums * 6 * $multiple;
            }elseif (in_array($val['play'], $leP)) {
                $noteNums = FJCalculation::fj115Fun_LeXuanPu($val['nums'], $ballNums[$val['play']]);
                if(in_array($val['play'], [201164,201167])){
                    $singleCost += $noteNums * 10 * $multiple;
                }  else {
                     $singleCost += $noteNums * 14 * $multiple;
                }
            }elseif (in_array($val['play'], $leD)) {
                $noteNums = FJCalculation::fj115Fun_LeXuanDan($val['nums'], $ballNums[$val['play']]);
                if($val['play'] == 201169){
                    $singleCost += $noteNums * 10 * $multiple;
                }  else {
                     $singleCost += $noteNums * 14 * $multiple;
        ...
        if ($total != $cost) {
         ...
         $insert = ['lottery_type' => $abbName, 'lottery_name' => $lotName, 'lottery_id' => $fjCode, 'play_code' => implode(',', $playCode), 'play_name' => implode(',', $playName), 'periods' => $periods,
...
//        print_r($insert);die;
    ...
    public function productSuborder($model) {
        ...
        foreach ($noteNums as $key => $nums) {
            $fun = 'fj115Note_' . $playCodes[$key];
            $ret = FJCalculation::$fun($nums);
            foreach ($ret as $k => $v) {
                $order[$n]['bet_val'] = $v;
