...
/**
 * ColorBoll controller for the `welfare` module
 */
class ColorBollController extends Controller {
...
//        }
        ...
        $betNums = '';
        $playName = [];
        $playParam = Constants::SSQ_PLAYNAME;
...
//        print_r($insert);die;
        //return OrderService::insertOrder($insert, $boolStr);
...
//        $model = LotteryOrder::findOne(["lottery_order_id" => $orderId]);
        ...
        foreach ($noteNums as $key => $val) {
            ...
            $fun = "ssqNote_" . $model->lottery_id;
            $ret = LogicApplication::$fun($numsArr, $playCodes[$key]);
            foreach ($ret as $k => $v) {
                $order[$n]["bet_val"] = $v;
